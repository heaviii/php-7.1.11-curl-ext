...
PHP_FUNCTION(curl_init_p)
{
    ...
    char *hkey = _msg_gen(domain_info, _MAX_URL_LEN_, "curl__%s_%s:%d", url_resource->scheme, url_resource->host, url_resource->port);
    ...
    if ((le = zend_hash_str_find_ptr(&EG(persistent_list), hkey, hkey_len)) != NULL) {
        ...
        zend_string *str = zend_string_init(hkey, hkey_len, 1);
        zval *zv;
        zval tmp;

        ZVAL_NEW_PERSISTENT_RES(&tmp, -1, new_le, le_pcurl);
        //GC_MAKE_PERSISTENT_LOCAL(Z_COUNTED(tmp));
//        Z_OBJ_APPLY_COUNT(Z_COUNTED(tmp));
        //GC_MAKE_PERSISTENT_LOCAL(str);

        zv = zend_hash_update(&EG(persistent_list), str, &tmp);

        zend_resource *ret  = Z_RES_P(zv);
